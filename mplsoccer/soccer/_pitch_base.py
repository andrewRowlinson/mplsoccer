...
class BasePitchSoccer(BasePitch):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            pad_top=pad_top,
            shade_middle=shade_middle,
            shade_color=shade_color,
            shade_alpha=shade_alpha,
            shade_zorder=shade_zorder,
        ...
        self.goal_left = np.array(
            [[self.dim.left, self.dim.goal_bottom], [self.dim.left, self.dim.goal_top]]
        )
        self.goal_right = np.array(
            [
                [self.dim.right, self.dim.goal_bottom],
    ...
    def _validation_checks(self):
        # pitch validation
        if self.pitch_type not in valid and not issubclass(
            type(self.pitch_type), BaseSoccerDims
        ):
            raise TypeError(
                f"Invalid argument: pitch_type should be in {valid} "
                "or a subclass of BaseSoccerDims."
            )
        if (
            self.pitch_length is None or self.pitch_width is None
        ) and self.pitch_type in size_varies:
            raise TypeError(
                "Invalid argument: pitch_length and pitch_width must be specified."
            )
        if (self.pitch_type not in size_varies) and (
            (self.pitch_length is not None) or (self.pitch_width is not None)
        ):
            msg = (
                f"Pitch length and widths are only used for {size_varies}"
        ...
        if self.goal_type not in valid_goal_type:
            raise TypeError(
                f"Invalid argument: goal_type should be in {valid_goal_type}"
            )

        valid_spot_type = ["circle", "square"]
        if self.spot_type not in valid_spot_type:
            raise TypeError(
                f"Invalid argument: spot_type should be in {valid_spot_type}"
            )
    ...
    def _set_background(self, ax):
        if self.pitch_color != "grass":
            ...
            ax.imshow(
                pitch_color, cmap=grass_cmap(), extent=self.extent, aspect=self.aspect
            )
    ...
    def _draw_pitch_markings(self, ax):
        ...
        xs_pbox_right = [
            self.dim.right,
            self.dim.penalty_area_right,
            self.dim.penalty_area_right,
            self.dim.right,
        ]
        xs_pbox_left = [
            self.dim.left,
            self.dim.penalty_area_left,
            self.dim.penalty_area_left,
            self.dim.left,
        ]
        ys_pbox = [
            self.dim.penalty_area_bottom,
            self.dim.penalty_area_bottom,
            self.dim.penalty_area_top,
            self.dim.penalty_area_top,
        ]
        ...
        xs_sixbox_right = [
            self.dim.right,
            self.dim.six_yard_right,
            self.dim.six_yard_right,
            self.dim.right,
        ]
        xs_sixbox_left = [
            self.dim.left,
            self.dim.six_yard_left,
            self.dim.six_yard_left,
            self.dim.left,
        ]
        ys_sixbox = [
            self.dim.six_yard_bottom,
            self.dim.six_yard_bottom,
            self.dim.six_yard_top,
            self.dim.six_yard_top,
        ]
    ...
    def _draw_circles_and_arcs(self, ax):
        ...
        self._draw_ellipse(
            ...
            self.dim.circle_diameter_width,
            **circ_prop,
        )
        self._draw_arc(
            ax,
            self.dim.penalty_left,
            self.dim.center_width,
            self.dim.circle_diameter_length,
            self.dim.circle_diameter_width,
            theta1=self.dim.arc1_theta1,
            theta2=self.dim.arc1_theta2,
            **circ_prop,
        )
        self._draw_arc(
            ax,
            self.dim.penalty_right,
            self.dim.center_width,
            self.dim.circle_diameter_length,
            self.dim.circle_diameter_width,
            theta1=self.dim.arc2_theta1,
            theta2=self.dim.arc2_theta2,
            **circ_prop,
        )
        ...
        if self.corner_arcs:
            ...
            for i, (x, y) in enumerate(corner_points):
                ...
                self._draw_arc(
                    ...
                    x,
                    y,
                    self.dim.corner_diameter_length,
    ...
    def _draw_goals(self, ax):
        if self.goal_type == "box":
            ...
            xs_left = [
                self.dim.left,
                self.dim.left - self.dim.goal_length,
                self.dim.left - self.dim.goal_length,
                self.dim.left,
            ]
            ys_left = [
                self.dim.goal_bottom,
                self.dim.goal_bottom,
                self.dim.goal_top,
                self.dim.goal_top,
            ]
            ...
            xs_right = [
                self.dim.right,
                self.dim.right + self.dim.goal_length,
                self.dim.right + self.dim.goal_length,
                self.dim.right,
            ]
            ys_right = [
            ...
            self._draw_line(
                ax,
                [self.dim.right, self.dim.right],
                [self.dim.goal_top, self.dim.goal_bottom],
                **line_prop,
            )
            self._draw_line(
                ax,
            ...
            posts = [
                [self.dim.right, self.dim.goal_bottom],
                [self.dim.right, self.dim.goal_top],
                [self.dim.left, self.dim.goal_bottom],
    ...
    def _draw_juego_de_posicion(self, ax):
        line_prop = {
            "linewidth": self.positional_linewidth,
            "color": self.positional_color,
            "alpha": self.positional_alpha,
        ...
        self._draw_line(
            ax,
            [self.dim.positional_x[1], self.dim.positional_x[1]],
            [self.dim.bottom, self.dim.top],
            **line_prop,
        )
        self._draw_line(
            ax,
            [self.dim.positional_x[5], self.dim.positional_x[5]],
            [self.dim.bottom, self.dim.top],
            **line_prop,
        )
        # short lines
        for coord in self.dim.positional_x[2:5]:
            self._draw_line(
                ax,
                [coord, coord],
                [self.dim.bottom, self.dim.penalty_area_bottom],
                **line_prop,
            )
            self._draw_line(
                ax,
                [coord, coord],
                [self.dim.top, self.dim.penalty_area_top],
                **line_prop,
            )
        ...
        self._draw_line(
            ...
            [self.dim.positional_y[1], self.dim.positional_y[1]],
            **line_prop,
        )
        self._draw_line(
            ax,
            [self.dim.penalty_area_left, self.dim.penalty_area_right],
            [self.dim.positional_y[2], self.dim.positional_y[2]],
            **line_prop,
        )
        self._draw_line(
            ax,
            [self.dim.penalty_area_left, self.dim.penalty_area_right],
            [self.dim.positional_y[3], self.dim.positional_y[3]],
            **line_prop,
        )
        self._draw_line(
            ax,
            [self.dim.left, self.dim.right],
            [self.dim.positional_y[4], self.dim.positional_y[4]],
            **line_prop,
        )
    ...
    def _draw_shade_middle(self, ax):
        shade_prop = {
            "fill": True,
            "facecolor": self.shade_color,
            "alpha": self.shade_alpha,
            "zorder": self.shade_zorder,
        }
        self._draw_rectangle(
            ax,
            self.dim.positional_x[2],
            self.dim.bottom,
            self.dim.positional_x[4] - self.dim.positional_x[2],
            self.dim.width,
            **shade_prop,
        )
    ...
    def formations_dataframe(self) -> pd.DataFrame:
        ...
        return pd.concat(
            [
                pd.DataFrame(
                    [formation.__dict__ for formation in self.dim.formations[key]]
                )
    ...
    def get_positions(self, line=5, second_striker=True) -> pd.DataFrame:
        ...
        if line == 5 and second_striker:
            return pd.DataFrame(
                {
                    key: value.__dict__
                    for key, value in self.dim.position_line5_with_ss.__dict__.items()
                }
            ).T
        if line == 4 and second_striker:
            return pd.DataFrame(
                {
                    key: value.__dict__
                    for key, value in self.dim.position_line4_with_ss.__dict__.items()
                }
            ).T
        if line == 5:
            return pd.DataFrame(
                {
                    key: value.__dict__
                    for key, value in self.dim.position_line5.__dict__.items()
                }
            ).T
        return pd.DataFrame(
            {
                key: value.__dict__
                for key, value in self.dim.position_line4.__dict__.items()
            }
        ).T
    ...
    def formation(
        ...
        **kwargs : All other keyword arguments are passed on to:
        ...
        >>> request = Request('https://upload.wikimedia.org/wikipedia/commons/b/b8/Messi_vs_Nigeria_2018.jpg')
        ...
        possible_position_list = [
            getattr(pos, self.pitch_type)
            if hasattr(pos, self.pitch_type)
            and getattr(pos, self.pitch_type) is not None
            else pos.name
        ...
        requires_positions = (
            xoffset is not None
            or yoffset is not None
            or self.pitch_type == "statsbomb"
            or kind in ("text", "image")
        )
        ...
        if positions is None and requires_positions:
            raise TypeError(
                "Missing 1 required argument: 'positions'. "
        ...
        if text is None and kind == "text":
            raise TypeError(
                "Missing 1 required argument: 'text'. "
        ...
        if image is None and kind == "image":
            raise TypeError(
                "Missing 1 required argument: 'image'. "
        ...
        for position in formation_positions:
            ...
            if self.pitch_type == "statsbomb":
                possible_positions = set(position.statsbomb).intersection(
                    set(positions)
                )
                ...
                self.pitch_type in ("wyscout", "opta")
                and getattr(position, self.pitch_type) is not None
            ):
            ...
            if requires_positions:
                ...
                position_idx = np.arange(len(positions))[
        ...
        if kind == "image":
            ...
            for i in range(len(formation_positions)):
                axes[position_names[i]] = self.inset_image(
                    x[i],
                    y[i],
                    sorted_image[i],
                    width=width,
        ...
        if kind == "pitch":
            ...
            for i in range(len(formation_positions)):
                axes[position_names[i]] = self.inset_axes(
                    x=x[i],
                    y=y[i],
                    height=height,
                    width=width,
                    aspect=1 / self.ax_aspect,
                    polar=False,
                    ax=ax,
                )
        ...
        raise NotImplementedError(
            f"kind = '{kind}' is not implemented. "
            "Valid arguments are 'scatter', 'image', 'axes', "
            "'pitch', or 'text'."
        )

    @copy_doc(bin_statistic_positional)
    def bin_statistic_positional(
        self, x, y, values=None, positional="full", statistic="count", normalize=False
    ):
