...
import numpy as np
from scipy.stats import binned_statistic_2d, circmean, binned_statistic_dd
...
@dataclass
class BinnedStatisticResultSonar:
    """ Dataclass for the bin_statistic_sonar results."""
    statistic: np.ndarray
    x_grid: np.ndarray
    y_grid: np.ndarray
...
def bin_statistic(x, y, values=None, dim=None, statistic='count',
    ...
    return BinnedStatisticResult(statistic, x_grid, y_grid, cx, cy, binnumber, inside)
...
def bin_statistic_sonar(x, y, angle, values=None, dim=None, statistic='count',
    ...
    inside = np.logical_and(~mask_x_out, ~mask_y_out)
    stats = BinnedStatisticResultSonar(statistic, x_grid, y_grid, angle_edge,
