...
def inset_image(
    x, y, image, width=None, height=None, vertical=False, ax=None, **kwargs
):
    ...
    **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.imshow.
    ...
    if height is not None and width is not None:
        raise TypeError(
            "Invalid argument: you must only give one of height or width not both"
        )
...
def inset_axes(
    ...
    **kwargs : All other keyword arguments are passed on to the inset_axes.
    ...
    if polar and height is not None and width is not None:
        raise TypeError(
            "Invalid argument: for polar axes provide only one of width or height"
        )
    if aspect is not None and width is not None and height is not None:
        raise TypeError(
            "Invalid argument: if using aspect you cannot use both width and height"
        )
    ...
    bbox = (x - width / 2, y - height / 2, width, height)
    inset_axes = ax.inset_axes(
        bbox, transform=ax.transData, xlim=xlim, ylim=ylim, polar=polar, **kwargs
    )
